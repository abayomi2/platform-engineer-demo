...
resource "aws_nat_gateway" "main_a" {
  ...
  subnet_id     = aws_subnet.public[keys(aws_subnet.public)[0]].id # Use the first public subnet by its AZ key
  tags = {
...
resource "aws_db_instance" "main" {
  ...
  engine               = "postgres"
  engine_version       = "17.2" # Changed to 15.5 for wider compatibility.
...
resource "null_resource" "eks_cluster_active_wait" {
  # This resource's creation only completes when the EKS cluster module is fully done,
  # including the cluster being in an ACTIVE state.
  depends_on = [
    module.eks_cluster
  ]

  # Optional: Add a local-exec provisioner to indicate it's running
...
module "jenkins_server" {
  ...
  eks_cluster_name   = module.eks_cluster.cluster_name # Pass EKS cluster name to Jenkins user_data script
  # Removed: depends_on = [module.eks_cluster] # Removed to break dependency cycle
}

# Output kubeconfig command for convenience (mostly for local debugging)
...
# NEW: 7. Configure EKS aws-auth ConfigMap to allow Jenkins IAM role access
# This directly addresses the "You must be logged in" error by mapping the IAM role to system:masters.
resource "kubernetes_config_map" "aws_auth_jenkins_role_mapping" {
  metadata {
    name      = "aws-auth"
    namespace = "kube-system"
  }
  data = {
    # This example OVERWRITES existing mapRoles.
    # For a real production setup with existing mappings, you'd use a more robust approach
    # like 'terraform-aws-modules/eks/aws//modules/aws-auth' or a strategic merge patch.
    # For this demo, this is acceptable.
    mapRoles = jsonencode([
      # EKS worker node role mapping (crucial for nodes to join cluster)
      {
        "rolearn"  = module.eks_cluster.node_group_role_arn
        "username" = "system:node:{{EC2PrivateDNSName}}"
        "groups"   = ["system:bootstrappers", "system:nodes"]
      },
      # Jenkins IAM role mapping to grant admin access
      {
        "rolearn"  = module.jenkins_server.jenkins_role_arn
        "username" = "jenkins"
        "groups"   = ["system:masters"] # Grants administrative access
      }
    ])
  }
  # Ensure this runs only after EKS is up and the Jenkins role is created
  # and the kubernetes provider is fully configured.
  depends_on = [
    null_resource.eks_cluster_active_wait, #module.eks_cluster.aws_eks_cluster.main, # This ensures the cluster is fully created and active
    module.jenkins_server,
    # The kubernetes provider implicitly depends on cluster_endpoint being available.
    # No explicit dependency needed for the provider if it's already using outputs.
  ]
}
